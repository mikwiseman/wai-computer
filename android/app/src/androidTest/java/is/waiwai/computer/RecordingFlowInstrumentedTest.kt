...
class RecordingFlowInstrumentedTest {
        ...
        recordingViewModel = RecordingViewModel(
            ...
            webSocketFactory = {
                TestWebSocketManager(
                    segments = listOf(
                        `is`.waiwai.computer.data.LiveTranscriptSegment(
    ...
    fun recording_screen_renders_live_transcript_and_finalizes_cleanly() {
        ...
        composeRule.onNodeWithTag(TestTags.RecordButton).performClick()
        composeRule.waitUntil(timeoutMillis = 5_000) {
            recordingViewModel.uiState.value.phase == `is`.waiwai.computer.recording.Phase.Idle
        }
        composeRule.waitUntil(timeoutMillis = 5_000) {
            fixture.backend.recordings.firstOrNull()?.status == `is`.waiwai.computer.data.RecordingStatus.Ready
        }
    }
}
