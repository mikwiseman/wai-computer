...
import `is`.waiwai.computer.library.RecordingDetailScreen
import `is`.waiwai.computer.library.RecordingDetailViewModel
import `is`.waiwai.computer.onboarding.GuestModeInfoSheet
import `is`.waiwai.computer.onboarding.OnboardingAuthChoice
import `is`.waiwai.computer.onboarding.OnboardingCarousel
...
fun WaiAndroidApp(
    ...
    val settings by container.settingsStore.settings.collectAsStateWithLifecycle(
        initialValue = `is`.waiwai.computer.data.AppSettings(
