...
public sealed class RecordingBackupStore
{
    ...
    public void MarkPermanentFailure(Guid id)
        => UpdateManifest(id, m => m with { IsPermanentFailure = true, UpdatedAt = DateTimeOffset.UtcNow });

    public void MarkAuthenticationRequired(Guid id)
        => UpdateManifest(id, m => m with { RequiresAuthentication = true, UpdatedAt = DateTimeOffset.UtcNow });
