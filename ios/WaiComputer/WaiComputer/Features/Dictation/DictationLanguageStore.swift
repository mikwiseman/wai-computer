...
/// Source of truth for the user's dictation/transcription language preference.
///
/// Ported verbatim from the macOS target (pure Foundation/UserDefaults/os) so
/// the iOS Settings + Recording surfaces share identical semantics.
///
...
/// on first read, and every persist mirrors back to that legacy key so the iOS
/// `RecordingViewModel` (which reads `transcriptionLanguage`) stays consistent.
@MainActor
...
/// frequency-of-use for the WaiComputer user base — English first, Russian
/// second, then a sweep of common picks. Ported verbatim from macOS.
