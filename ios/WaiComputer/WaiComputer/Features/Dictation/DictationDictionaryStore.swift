...
struct DictionaryWord: Identifiable, Codable, Hashable {
    ...
    /// "manual" or "learned" after accepting an edit-derived suggestion.
    var origin: String
    let createdAt: Date
    ...
    init(from decoder: Decoder) throws {
        ...
        // Older local caches predate `origin`.
...
final class DictationDictionaryStore: ObservableObject {
    ...
    /// instead of dropping the request on a duplicate.
    ...
    /// Edit an entry through the same delete/add sync paths used elsewhere.
    @discardableResult
    ...
    func loadScreenshotFixtures() {
        words = IOSScreenshotFixtures.dictionaryWords
        tombstones.removeAll()
        notifyRealtimeHintsChanged(reason: "dictionary_screenshot_fixtures")
    }
    #endif
