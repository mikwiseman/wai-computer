...
async def test_agent_capabilities_are_discoverable(client, auth_headers):
    ...
    assert "wai.brain.ask" in ids
    assert "wai.context.load" in ids
    assert "wai.respond.context" in ids
    assert "wai.respond.search" in ids
    ...
    assert "ask_brain" in contracts
    assert "load_context" in contracts
    assert "respond_from_context" in contracts
