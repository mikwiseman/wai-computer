...
@pytest.mark.asyncio
async def test_format_pending_actions_for_run_lists_only_pending(
    db_session: AsyncSession,
):
    user, _account = await _linked_account(db_session, "tg-pending@example.com", 9301)
    _agent, run = await _agent_with_run(db_session, user)
    with_recipient = _pending_action(user, run, recipient="Мама")
    no_preview = _pending_action(user, run, preview=None)
    executed = _pending_action(user, run, status="executed")
    db_session.add_all([with_recipient, no_preview, executed])
    await db_session.flush()

    text = await telegram_routes._format_pending_actions_for_run(db_session, run)

    assert text.startswith("Нужно подтверждение:")
    assert f"/approve {with_recipient.id}" in text
    assert f"/approve_always {with_recipient.id}" in text
    assert f"/reject {with_recipient.id}" in text
    assert " · Мама" in text
    assert "Послать привет" in text
    # Empty preview falls back to the tool name.
    assert "send_message_telegram · send" in text
    assert str(executed.id) not in text

    # A run with no pending actions formats to an empty string.
    _agent2, run2 = await _agent_with_run(db_session, user)
    assert await telegram_routes._format_pending_actions_for_run(db_session, run2) == ""


# ---------------------------------------------------------------------------
# _format_wai_run_reply (lines 2874-2893)
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_format_wai_run_reply_failed_artifacts_and_fallback(
    db_session: AsyncSession,
):
    user, _account = await _linked_account(db_session, "tg-run-reply@example.com", 9302)

    _agent, failed = await _agent_with_run(db_session, user, status="failed", error="boom")
    assert (
        await telegram_routes._format_wai_run_reply(db_session, failed)
        == "Не получилось выполнить задачу Wai: boom"
    )

    _agent2, failed_blank = await _agent_with_run(db_session, user, status="failed")
    assert "ошибка агента" in await telegram_routes._format_wai_run_reply(
        db_session, failed_blank
    )

    _agent3, done = await _agent_with_run(
        db_session,
        user,
        status="done",
        result={
            ...
            "artifacts": [
                "bogus",
                {"title": "Отчет", "kind": "note"},
                {"item_id": "abc-123"},
            ],
        },
    )
    action = _pending_action(user, done, preview="Отправить отчет")
    db_session.add(action)
    await db_session.flush()
    text = await telegram_routes._format_wai_run_reply(db_session, done)
    assert "Сделал." in text
    assert "Артефакты:" in text
    assert "- Отчет (note)" in text
    assert "- abc-123 (item)" in text
    assert "Нужно подтверждение:" in text
    assert f"/approve {action.id}" in text

    # Artifacts without output text become the whole reply.
    _agent4, artifacts_only = await _agent_with_run(
        db_session,
        user,
        status="done",
        result={"artifacts": [{"title": "Таблица", "kind": "sheet"}]},
    )
    text2 = await telegram_routes._format_wai_run_reply(db_session, artifacts_only)
    assert text2.startswith("Артефакты:")
    assert "- Таблица (sheet)" in text2

    # No text at all falls back to the status line.
    _agent5, running = await _agent_with_run(db_session, user, status="running")
    assert (
        await telegram_routes._format_wai_run_reply(db_session, running)
        == f"Wai run {running.id} status: running"
    )


# ---------------------------------------------------------------------------
# _handle_callback_query consent / delete dispatch (2821-2829, 2837-2845)
# ---------------------------------------------------------------------------
