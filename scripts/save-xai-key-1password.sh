#!/usr/bin/env bash
# Save the xAI API key to 1Password without putting the secret in shell
# history, command arguments, or a temporary file.

set -euo pipefail

VAULT="${OP_VAULT:-Development}"
ITEM_TITLE="${OP_ITEM_TITLE:-xAI}"
...
read -r -s -p "New xAI API key: " XAI_API_KEY
echo
...
set_field("username", "xAI")
set_field("hostname", "api.x.ai")
set_field("type", "API key", "MENU")
set_field(
    "notesPlain",
    "Runtime key for WaiComputer xAI summary text-to-speech.",
)
...
echo "Saved xAI key to op://${VAULT}/${ITEM_TITLE}/${FIELD_LABEL}"
