...
BACKEND_URL = os.environ.get("WAICOMPUTER_BACKEND_URL", "https://wai.computer").rstrip("/")
SAMPLE_RATE = 16000
CHUNK_MS = 50
...
class ScenarioResult:
    ...
    # cleaned_transcript is the backend dictation cleanup output; clean_wer
    # compares it against scenario.cleaned_expected; cleanup_ms is the
    # wall-clock round-trip to the production cleanup endpoint.
...
def load_test_access_token() -> str:
    """Return the bearer token used for backend cleanup endpoint checks."""
    candidate = Path.home() / ".config" / "waicomputer" / "test-token.env"
    if not candidate.exists():
        raise RuntimeError(
            f"test access token missing. Expected at {candidate} (mode 0600).\n"
            "Fetch or mint a production test token and save it as ACCESS_TOKEN=..."
        )
    raw = candidate.read_text().strip()
    if "=" in raw:
        raw = raw.split("=", 1)[1].strip().strip("\"'")
    if not raw:
        raise RuntimeError(f"test access token at {candidate} is empty")
    return raw


def clean_with_backend(access_token: str, text: str) -> tuple[str, float]:
    """Call the production backend dictation cleanup endpoint.
    ...
    req = urllib.request.Request(
        f"{BACKEND_URL}/api/dictation/cleanup",
    ...
    cleaned = str(payload.get("text") or "").strip()
    if not cleaned:
        raise RuntimeError(f"backend cleanup returned empty text: keys={list(payload.keys())}")
...
def load_inworld_key() -> str:
    ...
    if not candidate.exists():
        raise RuntimeError(
            ...
            "Fetch with: ssh root@157.180.47.68 'grep ^INWORLD_API_KEY= /etc/waicomputer/backend.env' "
...
async def run_scenario(
    ...
    if scenario.get("apply_cleanup") and transcript:
        try:
            ...
            cleaned_transcript, cleanup_ms = clean_with_backend(access_token, transcript)
