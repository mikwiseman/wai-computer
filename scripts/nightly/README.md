...
Each iteration generates fresh TTS audio from Inworld, streams it through the same STT pipeline WaiSay uses in production, computes word error rate (WER) and latency against the source text, then writes a structured report.
...
- **Tier 2 (opt-in via `NIGHTLY_TIER2=1`)**: BlackHole virtual mic + real WaiSay.app launch + hot-key trigger + clipboard verification. Requires `BlackHole 2ch` driver installed (one-time `brew install --cask blackhole-2ch` + reboot).
...
1. `cd /Users/mikwiseman/Documents/Code/wai-say && git pull --ff-only origin main`
...
7. Maintain coverage ≥95%: `cd backend && pytest -q --cov`, `cd web && pnpm test:unit`, `cd shared/WaiSayKit && swift test -q`.
...
- `INWORLD_API_KEY` is read from `~/.config/waisay/inworld.env` (mode 0600, fetched once from prod `/etc/waisay/backend.env`).
- Production STT session minting uses `https://say.waiwai.is/api/transcription/session` with a test access token from `~/.config/waisay/test-token.env` (optional — falls back to direct Inworld auth).
