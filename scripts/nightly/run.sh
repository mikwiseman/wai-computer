...
if [[ ! -f "$HOME/.config/waisay/inworld.env" ]]; then
  ...
  echo "Fetch with: ssh <release-user>@<release-host> 'grep ^INWORLD_API_KEY= /etc/waisay/backend.env' > $HOME/.config/waisay/inworld.env && chmod 600 $HOME/.config/waisay/inworld.env" >&2
  exit 2
fi
