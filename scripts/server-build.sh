...
# synced to <remote-root>. It keeps all Docker image builds on the server and
# uses <remote-env-file> as the only production runtime env file.
set -euo pipefail

PROD_ROOT="${PROD_ROOT:-<remote-root>}"
