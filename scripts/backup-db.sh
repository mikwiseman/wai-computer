...
# Crontab: 0 3 * * * <remote-root>/scripts/backup-db.sh

set -euo pipefail

BACKUP_DIR="<remote-root>/backups"
RETENTION_DAYS=14
...
POSTGRES_USER=$(grep '^POSTGRES_USER=' <remote-root>/backend/.env | cut -d= -f2)
POSTGRES_DB=$(grep '^POSTGRES_DB=' <remote-root>/backend/.env | cut -d= -f2)
