#!/usr/bin/env bash
# Rotate the production xAI API key in the root-owned runtime env file on
...
read -r -s -p "New XAI_API_KEY: " XAI_API_KEY
echo
...
' \
  | ssh \
      ...
      "set -euo pipefail;
       ...
       rm -f \"\$tmp\";
       grep -Eq '^XAI_API_KEY=.+' \"${REMOTE_ENV_FILE}\""

echo "Rotated XAI_API_KEY in ${VPS_HOST}:${REMOTE_ENV_FILE}"
