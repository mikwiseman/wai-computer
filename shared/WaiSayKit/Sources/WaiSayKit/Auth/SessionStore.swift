...
private let log = Logger(subsystem: "com.waisay.auth", category: "session-store")
...
/// at-rest protection.
///
/// `SessionStore` writes to `<Application Support>/WaiSay/session.json` with
...
public final class SessionStore: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.waisay.auth.sessionstore")
    private let fileManager: FileManager
    private let directoryName: String

    private init() {
        self.fileManager = .default
        self.directoryName = "WaiSay"
    }
