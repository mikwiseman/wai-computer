...
- Production DMG releases must use `MACOS_RELEASE_STRICT=1 scripts/build-macos-dmg.sh` followed by `VPS_USER=<release-user> scripts/publish-macos-dmg.sh`, or `VPS_USER=<release-user> fastlane mac upload_all`.
