...
- Local release fallback (CI unavailable): `MACOS_RELEASE_STRICT=1 scripts/build-macos-dmg.sh` then `VPS_USER=<release-user> scripts/publish-macos-dmg.sh`, or `VPS_USER=<release-user> fastlane mac upload_all`.
