...
- [~] Web: Brain section — nav (Library → Brain) + route + **Index view** DONE (categorized entities from `getBrainGraph`, stats, honest empty + error/retry); `getBrainGraph` client + `BrainGraph` types + `BrainPanel`. PENDING: Graph tab (`react-force-graph-2d` — new dep, verify version, dynamic ssr:false, ego default, items toggle), Wiki tab (needs `GET /api/entities/{id}/page`), tri-view tab switcher, and the authenticated browser verification (login → populate entities → check `/ru`).
