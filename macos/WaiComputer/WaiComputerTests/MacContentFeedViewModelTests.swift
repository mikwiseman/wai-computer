...
final class MacContentFeedViewModelTests: XCTestCase {
    ...
    func testMacSchemesBoardExposesHistoryAndDuplicateControls() throws {
        ...
        XCTAssertTrue(source.contains("@State private var selectedItemIds: [String] = []"))
        XCTAssertTrue(source.contains("@State private var marqueeStart: SchemePosition?"))
        XCTAssertTrue(source.contains("@State private var lassoPoints: [SchemePosition] = []"))
        XCTAssertTrue(source.contains("@State private var sourceDrag: ItemDragState?"))
        XCTAssertTrue(source.contains("@State private var commentDrag: ItemDragState?"))
        XCTAssertTrue(source.contains("@State private var resizeDrag: ResizeDragState?"))
        XCTAssertTrue(source.contains("private let defaultGridSize = 40.0"))
        XCTAssertTrue(source.contains("private struct MultiItemDragState"))
        XCTAssertTrue(source.contains("private struct ResizeDragState"))
        XCTAssertTrue(source.contains("private struct ProjectionSourceSummary"))
        ...
        XCTAssertTrue(source.contains("private func pinProjectionSources()"))
        XCTAssertTrue(source.contains("private func sourceGesture(for source: SchemeCanvasSourceBlock) -> some Gesture"))
        XCTAssertTrue(source.contains("private func commentGesture(for comment: SchemeCanvasComment) -> some Gesture"))
        XCTAssertTrue(source.contains("case highlighter"))
        XCTAssertTrue(source.contains("case eraser"))
        XCTAssertTrue(source.contains("case lasso"))
        XCTAssertTrue(source.contains("private func eraseStrokes(at point: SchemePosition)"))
        XCTAssertTrue(source.contains("private func strokeContains(_ stroke: SchemeStroke, point: SchemePosition) -> Bool"))
        ...
        XCTAssertTrue(source.contains("private func selectionIds(in rect: CGRect) -> [String]"))
        XCTAssertTrue(source.contains("private func selectionIds(inLasso points: [SchemePosition]) -> [String]"))
        XCTAssertTrue(source.contains("private func pointInPolygon(_ point: SchemePosition, polygon: [SchemePosition]) -> Bool"))
        XCTAssertTrue(source.contains("private func drawLasso(context: GraphicsContext, size: CGSize)"))
        XCTAssertTrue(source.contains("private func beginMultiDragIfNeeded(id: String, translation: CGSize) -> Bool"))
        XCTAssertTrue(source.contains("private func itemRect(in candidate: SchemeCanvasLayout, id: String) -> CGRect?"))
        XCTAssertTrue(source.contains("private func drawGrid(context: GraphicsContext, size: CGSize)"))
        XCTAssertTrue(source.contains("private var orderedFrames: [SchemeCanvasFrame]"))
        XCTAssertTrue(source.contains("private func focusFrame(_ frame: SchemeCanvasFrame)"))
        XCTAssertTrue(source.contains("private func focusAdjacentFrame(offset: Int)"))
        XCTAssertTrue(source.contains("private var boardContentBounds: CGRect?"))
        XCTAssertTrue(source.contains("private func fitBoard()"))
        XCTAssertTrue(source.contains("private func focusBoardOverviewPoint(_ point: SchemePosition)"))
        XCTAssertTrue(source.contains("MacSchemeOverviewMap("))
        XCTAssertTrue(source.contains("private var boardOutlineItems: [MacSchemeOutlineItem]"))
        XCTAssertTrue(source.contains("private func focusOutlineItem(_ item: MacSchemeOutlineItem)"))
        XCTAssertTrue(source.contains("MacSchemeOutlinePanel("))
        XCTAssertTrue(source.contains("MacSchemeCommentsPanel("))
        XCTAssertTrue(source.contains("MacSchemeFacilitationPanel("))
        XCTAssertTrue(source.contains("private var unresolvedCommentCount: Int"))
        XCTAssertTrue(source.contains("private var selectedVoteTargetId: String?"))
        XCTAssertTrue(source.contains("private func startVotingSession()"))
        XCTAssertTrue(source.contains("private func addVote(to itemId: String)"))
        XCTAssertTrue(source.contains("private func clearVotingSession()"))
        XCTAssertTrue(source.contains("private func startTimer()"))
        XCTAssertTrue(source.contains("private func pauseTimer()"))
        XCTAssertTrue(source.contains("private func stopTimer()"))
        XCTAssertTrue(source.contains("private func addComment(at point: SchemePosition)"))
        XCTAssertTrue(source.contains("private func toggleCommentResolved(_ commentId: String)"))
        XCTAssertTrue(source.contains("private func updateComment(_ id: String, mutate: (inout SchemeCanvasComment) -> Void)"))
        XCTAssertTrue(source.contains("private var presentationFrame: SchemeCanvasFrame?"))
        XCTAssertTrue(source.contains("private func startPresentation()"))
        XCTAssertTrue(source.contains("private func exitPresentation()"))
        XCTAssertTrue(source.contains("private func advancePresentation(offset: Int)"))
        XCTAssertTrue(source.contains("private func snapPosition(_ position: SchemePosition, isBypassed: Bool = false) -> SchemePosition"))
        XCTAssertTrue(source.contains("private func snapRect(_ rect: (x: Double, y: Double, width: Double, height: Double), handle: ResizeHandle?) -> (x: Double, y: Double, width: Double, height: Double)"))
        XCTAssertTrue(source.contains("private func resizeHandleOverlay(id: String, kind: ResizableItemKind) -> some View"))
        XCTAssertTrue(source.contains("private func resizeGesture(id: String, kind: ResizableItemKind, handle: ResizeHandle) -> some Gesture"))
        XCTAssertTrue(source.contains("private func resizedLayout(from origin: SchemeCanvasLayout, id: String, kind: ResizableItemKind, handle: ResizeHandle, translation: CGSize) -> SchemeCanvasLayout"))
        XCTAssertTrue(source.contains("private func isItemLocked(_ id: String) -> Bool"))
        XCTAssertTrue(source.contains("private enum ResizeHandle"))
        XCTAssertTrue(source.contains("private enum ResizableItemKind"))
        XCTAssertTrue(source.contains("private enum LayerAction"))
        XCTAssertTrue(source.contains("RoundedRectangle(cornerRadius: 3)"))
        ...
        XCTAssertTrue(source.contains("Image(systemName: layout.snapToGrid ? \"grid.circle.fill\" : \"grid.circle\")"))
        XCTAssertTrue(source.contains("Stepper(value: $layout.gridSize"))
        XCTAssertTrue(source.contains("Image(systemName: \"rectangle.stack\")"))
        XCTAssertTrue(source.contains("Image(systemName: \"chevron.right\")"))
        XCTAssertTrue(source.contains("Image(systemName: \"text.bubble\")"))
        XCTAssertTrue(source.contains("Image(systemName: \"arrow.down.right.and.arrow.up.left\")"))
        ...
        XCTAssertTrue(source.contains("MacSchemeSourceBlockView(source: source)"))
        XCTAssertTrue(source.contains("MacSchemeCommentPin(comment: comment"))
        XCTAssertTrue(source.contains("MacSchemeVoteBadge(count: vote.count)"))
        XCTAssertTrue(source.contains(".accessibilityIdentifier(\"scheme-vote-\\(vote.itemId)\")"))
        XCTAssertTrue(source.contains(".accessibilityIdentifier(\"scheme-comment-\\(comment.id)\")"))
        XCTAssertTrue(source.contains(".accessibilityIdentifier(\"scheme-source-\\(source.id)\")"))
        ...
        XCTAssertTrue(source.contains(".disabled(unpinnedProjectionSources.isEmpty)"))
